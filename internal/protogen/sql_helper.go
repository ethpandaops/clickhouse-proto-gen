...
func (g *Generator) writeDateTimeFilterCases(sb *strings.Builder, columnName, filterType, indent string) {
	...
	case "NullableUInt32Filter":
		...
		fmt.Fprintf(sb, "%s\tif filter.IsNull {\n", indent)
		fmt.Fprintf(sb, "%s\t\tqb.AddIsNullCondition(\"%s\")\n", indent, columnName)
		fmt.Fprintf(sb, "%s\t}\n", indent)

		fmt.Fprintf(sb, "%scase *NullableUInt32Filter_IsNotNull:\n", indent)
		fmt.Fprintf(sb, "%s\tif filter.IsNotNull {\n", indent)
		fmt.Fprintf(sb, "%s\t\tqb.AddIsNotNullCondition(\"%s\")\n", indent, columnName)
		fmt.Fprintf(sb, "%s\t}\n", indent)

		fmt.Fprintf(sb, "%scase *NullableUInt32Filter_Eq:\n", indent)
		fmt.Fprintf(sb, "%s\tif filter.Eq != nil {\n", indent)
		fmt.Fprintf(sb, "%s\t\tqb.AddCondition(\"%s\", \"=\", DateTimeValue{filter.Eq.GetValue()})\n", indent, columnName)
		fmt.Fprintf(sb, "%s\t}\n", indent)

		fmt.Fprintf(sb, "%scase *NullableUInt32Filter_Ne:\n", indent)
		fmt.Fprintf(sb, "%s\tif filter.Ne != nil {\n", indent)
		fmt.Fprintf(sb, "%s\t\tqb.AddCondition(\"%s\", \"!=\", DateTimeValue{filter.Ne.GetValue()})\n", indent, columnName)
		fmt.Fprintf(sb, "%s\t}\n", indent)

		fmt.Fprintf(sb, "%scase *NullableUInt32Filter_Lt:\n", indent)
		fmt.Fprintf(sb, "%s\tif filter.Lt != nil {\n", indent)
		fmt.Fprintf(sb, "%s\t\tqb.AddCondition(\"%s\", \"<\", DateTimeValue{filter.Lt.GetValue()})\n", indent, columnName)
		fmt.Fprintf(sb, "%s\t}\n", indent)

		fmt.Fprintf(sb, "%scase *NullableUInt32Filter_Lte:\n", indent)
		fmt.Fprintf(sb, "%s\tif filter.Lte != nil {\n", indent)
		fmt.Fprintf(sb, "%s\t\tqb.AddCondition(\"%s\", \"<=\", DateTimeValue{filter.Lte.GetValue()})\n", indent, columnName)
		fmt.Fprintf(sb, "%s\t}\n", indent)

		fmt.Fprintf(sb, "%scase *NullableUInt32Filter_Gt:\n", indent)
		fmt.Fprintf(sb, "%s\tif filter.Gt != nil {\n", indent)
		fmt.Fprintf(sb, "%s\t\tqb.AddCondition(\"%s\", \">\", DateTimeValue{filter.Gt.GetValue()})\n", indent, columnName)
		fmt.Fprintf(sb, "%s\t}\n", indent)

		fmt.Fprintf(sb, "%scase *NullableUInt32Filter_Gte:\n", indent)
		fmt.Fprintf(sb, "%s\tif filter.Gte != nil {\n", indent)
		fmt.Fprintf(sb, "%s\t\tqb.AddCondition(\"%s\", \">=\", DateTimeValue{filter.Gte.GetValue()})\n", indent, columnName)
		fmt.Fprintf(sb, "%s\t}\n", indent)

		fmt.Fprintf(sb, "%scase *NullableUInt32Filter_Between:\n", indent)
		fmt.Fprintf(sb, "%s\tif filter.Between != nil {\n", indent)
		fmt.Fprintf(sb, "%s\t\tqb.AddBetweenCondition(\"%s\", DateTimeValue{filter.Between.Min}, DateTimeValue{filter.Between.Max.GetValue()})\n", indent, columnName)
		fmt.Fprintf(sb, "%s\t}\n", indent)

		fmt.Fprintf(sb, "%scase *NullableUInt32Filter_In:\n", indent)
		fmt.Fprintf(sb, "%s\tif filter.In != nil && len(filter.In.Values) > 0 {\n", indent)
		fmt.Fprintf(sb, "%s\t\tconverted := make([]interface{}, len(filter.In.Values))\n", indent)
		fmt.Fprintf(sb, "%s\t\tfor i, v := range filter.In.Values {\n", indent)
		fmt.Fprintf(sb, "%s\t\t\tif v != nil {\n", indent)
		fmt.Fprintf(sb, "%s\t\t\t\tconverted[i] = DateTimeValue{v.GetValue()}\n", indent)
		fmt.Fprintf(sb, "%s\t\t\t}\n", indent)
		fmt.Fprintf(sb, "%s\t\t}\n", indent)
		fmt.Fprintf(sb, "%s\t\tqb.AddInCondition(\"%s\", converted)\n", indent, columnName)
		fmt.Fprintf(sb, "%s\t}\n", indent)
		...
		fmt.Fprintf(sb, "%s\tif filter.NotIn != nil && len(filter.NotIn.Values) > 0 {\n", indent)
		fmt.Fprintf(sb, "%s\t\tconverted := make([]interface{}, len(filter.NotIn.Values))\n", indent)
		fmt.Fprintf(sb, "%s\t\tfor i, v := range filter.NotIn.Values {\n", indent)
		fmt.Fprintf(sb, "%s\t\t\tif v != nil {\n", indent)
		fmt.Fprintf(sb, "%s\t\t\t\tconverted[i] = DateTimeValue{v.GetValue()}\n", indent)
	...
	case "NullableUInt64Filter":
		...
		fmt.Fprintf(sb, "%s\tif filter.Eq != nil {\n", indent)
		fmt.Fprintf(sb, "%s\t\tqb.AddCondition(\"%s\", \"=\", DateTime64Value{filter.Eq.GetValue()})\n", indent, columnName)
		fmt.Fprintf(sb, "%s\t}\n", indent)

		fmt.Fprintf(sb, "%scase *NullableUInt64Filter_Ne:\n", indent)
		fmt.Fprintf(sb, "%s\tif filter.Ne != nil {\n", indent)
		fmt.Fprintf(sb, "%s\t\tqb.AddCondition(\"%s\", \"!=\", DateTime64Value{filter.Ne.GetValue()})\n", indent, columnName)
		fmt.Fprintf(sb, "%s\t}\n", indent)

		fmt.Fprintf(sb, "%scase *NullableUInt64Filter_Lt:\n", indent)
		fmt.Fprintf(sb, "%s\tif filter.Lt != nil {\n", indent)
		fmt.Fprintf(sb, "%s\t\tqb.AddCondition(\"%s\", \"<\", DateTime64Value{filter.Lt.GetValue()})\n", indent, columnName)
		fmt.Fprintf(sb, "%s\t}\n", indent)

		fmt.Fprintf(sb, "%scase *NullableUInt64Filter_Lte:\n", indent)
		fmt.Fprintf(sb, "%s\tif filter.Lte != nil {\n", indent)
		fmt.Fprintf(sb, "%s\t\tqb.AddCondition(\"%s\", \"<=\", DateTime64Value{filter.Lte.GetValue()})\n", indent, columnName)
		fmt.Fprintf(sb, "%s\t}\n", indent)

		fmt.Fprintf(sb, "%scase *NullableUInt64Filter_Gt:\n", indent)
		fmt.Fprintf(sb, "%s\tif filter.Gt != nil {\n", indent)
		fmt.Fprintf(sb, "%s\t\tqb.AddCondition(\"%s\", \">\", DateTime64Value{filter.Gt.GetValue()})\n", indent, columnName)
		fmt.Fprintf(sb, "%s\t}\n", indent)

		fmt.Fprintf(sb, "%scase *NullableUInt64Filter_Gte:\n", indent)
		fmt.Fprintf(sb, "%s\tif filter.Gte != nil {\n", indent)
		fmt.Fprintf(sb, "%s\t\tqb.AddCondition(\"%s\", \">=\", DateTime64Value{filter.Gte.GetValue()})\n", indent, columnName)
		fmt.Fprintf(sb, "%s\t}\n", indent)
		...
		fmt.Fprintf(sb, "%s\t\tfor i, v := range filter.In.Values {\n", indent)
		fmt.Fprintf(sb, "%s\t\t\tif v != nil {\n", indent)
		fmt.Fprintf(sb, "%s\t\t\t\tconverted[i] = DateTime64Value{v.GetValue()}\n", indent)
		fmt.Fprintf(sb, "%s\t\t\t}\n", indent)
		fmt.Fprintf(sb, "%s\t\t}\n", indent)
		fmt.Fprintf(sb, "%s\t\tqb.AddInCondition(\"%s\", converted)\n", indent, columnName)
		fmt.Fprintf(sb, "%s\t}\n", indent)
